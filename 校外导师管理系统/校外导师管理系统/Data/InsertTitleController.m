...
-(void)deleteTitleInServletWithTitle:(NSString *)title {
    ...
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    //获取url
    [ConnectURL appendUrl:@"DeleteInterlayerServlet"];
    NSString *url = [ConnectURL shareURL];
    //数据
    NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:title, @"name", nil];
    //删除
    [manager POST:url parameters:data progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteResult" object:responseObject];
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        [SVProgressHUD dismiss];
